...
import { ANIMATION_STUDIO_MANIFEST_ROUTE, ANIMATION_STUDIO_SAVE_ROUTE } from "../animation/persistence";
...
declare global {
  interface Window {
    AnimationRuntimeBridge?: {
      ...
      setLegacyBaseClip: (rigRoot: Group, rigId: string, clipId: string, frameNowMs: number) => void;
      requestLegacyActionClip: (rigRoot: Group, rigId: string, clipId: string, options: { startedAtMs: number; startKey: string; priority: number }) => void;
...
export function exposeAnimationBridge(): void {
  window.AnimationRuntimeBridge = {
    ...
    setLegacyBaseClip: (rigRoot, rigId, clipId, frameNowMs) => {
      const controller = ensureRigAnimationController(rigRoot, rigId);
      setBaseAnimationClip(controller, clipId, frameNowMs);
    },
    requestLegacyActionClip: (rigRoot, rigId, clipId, options) => {
      const controller = ensureRigAnimationController(rigRoot, rigId);
      requestActionAnimationClip(controller, clipId, options.startedAtMs, options.startKey, options.priority);
    },
    applyLegacyFrame: (rigRoot, rigId, frameNowMs) => {
      const controller = ensureRigAnimationController(rigRoot, rigId);
      return applyAnimationControllerFrame(rigRoot, controller, frameNowMs, {
