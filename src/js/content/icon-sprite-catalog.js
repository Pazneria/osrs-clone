(function () {
    ...
    function resolveSpriteKey(spriteKey) {
        ...
        if (spriteMarkupByKey[normalized]) {
            return {
        ...
        const fallbackKey = normalizeSpriteKey(fallbackSpriteKeyByKey[normalized]);
        if (fallbackKey && spriteMarkupByKey[fallbackKey]) {
            return {
                requestedKey: normalized,
    ...
    function resolveSpriteMarkup(spriteKey) {
        const resolution = resolveSpriteKey(spriteKey);
        return {
            requestedKey: resolution.requestedKey,
            resolvedKey: resolution.resolvedKey,
            usedFallback: resolution.usedFallback,
            markup: resolution.resolvedKey ? (spriteMarkupByKey[resolution.resolvedKey] || '') : ''
        };
    }

    function incrementCounter(counterMap, key) {
        if (!key) return;
        if (!Object.prototype.hasOwnProperty.call(counterMap, key)) {
            counterMap[key] = 0;
        }
        counterMap[key] += 1;
    }

    function collectSpriteReferenceSummary(itemDefs) {
        const defs = itemDefs && typeof itemDefs === 'object' ? itemDefs : {};
        const ids = Object.keys(defs);
        const fallbackUsageByKey = {};
        const missingByKey = {};
        let totalSpriteReferences = 0;

        for (let i = 0; i < ids.length; i++) {
            const itemId = ids[i];
            const itemDef = defs[itemId];
            const icon = itemDef && itemDef.icon;
            if (!icon || icon.kind !== 'sprite') continue;
            totalSpriteReferences += 1;
            const resolved = resolveSpriteMarkup(icon.key);
            if (!resolved.markup) {
                incrementCounter(missingByKey, icon.key || '(empty)');
                continue;
            }
            if (resolved.usedFallback) {
                incrementCounter(fallbackUsageByKey, resolved.requestedKey);
            }
        }

        return {
            totalSpriteReferences,
            fallbackUsageByKey,
            missingByKey
        };
    }

    window.IconSpriteCatalog = {
        spriteMarkupByKey,
        fallbackSpriteKeyByKey,
        resolveSpriteKey,
        resolveSpriteMarkup,
        collectSpriteReferenceSummary
    };
})();
