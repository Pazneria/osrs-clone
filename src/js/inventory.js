// --- Bank & Drag/Drop Logic ---
        ...
        function buildSkillPanelTimeline(skillName) {
            const runtime = getSkillPanelRuntime();
            return runtime && typeof runtime.buildSkillPanelTimeline === 'function'
                ? runtime.buildSkillPanelTimeline({
                    skillName,
        ...
        function isSkillPanelRecipeUnlock(unlockEntry) {
            if (!unlockEntry || !unlockEntry.recipe) return false;
            const runtime = getSkillPanelRuntime();
            const recipeType = runtime && typeof runtime.SKILL_PANEL_RECIPE_UNLOCK_TYPE === 'string'
                ? runtime.SKILL_PANEL_RECIPE_UNLOCK_TYPE
                : 'recipe';
            return unlockEntry.unlockType === recipeType;
        }
        ...
        function renderSkillPanelTimelineLegacy(skillName, currentLevel) {
            ...
            for (let i = 0; i < entries.length; i++) {
                ...
                for (let j = 0; j < entry.unlocks.length; j++) {
                    ...
                    const isRecipe = isSkillPanelRecipeUnlock(unlock);
