...
- [ ] FIREMAKING-005: Runtime fire object/state implementation (lit/expired/ashes lifecycle, timers, tile occupancy rules).
- [ ] FIREMAKING-006: Ignition attempt resolution wiring (per-tick success check, consume-on-success behavior, repeated fire chain movement).

## Next
- [ ] FIREMAKING-007: Ashes ground-item behavior integration (stacking, pickup, despawn timing, non-blocking tile interaction).
- [ ] FIREMAKING-008: Cooking compatibility pass so any lit fire can be targeted as a valid cooking source.
