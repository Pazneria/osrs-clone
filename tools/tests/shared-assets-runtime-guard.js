...
function run() {
  ...
  assert(sharedAssetsSource.includes("function addWhiteVertexColorAttribute(THREE, geometry)"), "shared asset runtime should keep a reusable white vertex-color base for instanced detail tinting");
  assert(sharedAssetsSource.includes("geometry.setAttribute('color', new THREE.Float32BufferAttribute(colors, 3));"), "instanced landscape detail geometry should include vertex colors so tinting cannot render black");
  assert(sharedAssetsSource.includes("function createGrassTuftGeometry(THREE)"), "shared asset runtime should own low-poly grass tuft geometry");
  assert(sharedAssetsSource.includes("sharedGeometries.grassTuft = createGrassTuftGeometry(THREE);"), "shared asset runtime should expose reusable grass tuft geometry");
  assert(sharedAssetsSource.includes("sharedGeometries.bushClump = addWhiteVertexColorAttribute"), "shared asset runtime should expose reusable tinted bush clump geometry");
  assert(sharedAssetsSource.includes("sharedGeometries.groundPebble = addWhiteVertexColorAttribute"), "shared asset runtime should expose reusable tinted ground pebble geometry");
  ...
  assert(sharedAssetsSource.includes("sharedMaterials.grassTuft = new THREE.MeshLambertMaterial"), "shared asset runtime should own grass tuft material setup");
  assert(sharedAssetsSource.includes("sharedMaterials.bushLeaves = new THREE.MeshLambertMaterial"), "shared asset runtime should own bush material setup");
  assert(sharedAssetsSource.includes("sharedMaterials.groundPebble = new THREE.MeshLambertMaterial"), "shared asset runtime should own ground pebble material setup");
